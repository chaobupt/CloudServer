...
void example_ckks_encoder()
{
    ...
    */
    ...
    CKKS scheme. The CKKSEncoder encodes vectors of real or complex numbers into		CKKSEncoder将实数向量encode进Plaintext 对象中
    Plaintext objects, which can subsequently be encrypted. At a high level this	CKKSEncoder将实数或复数的向量编码成明文对象，这些对象随后可以被加密。
    ...
    */
    ...
    cout << "Number of slots: " << slot_count << endl; //4096

    /*
    We create a small vector to encode; the CKKSEncoder will implicitly pad it		我们创建一个小的向量进行编码;在编码时，CKKSEncoder将隐式地用0填充到全尺寸(poly_modulus_degree / 2)。
    ...
    */
    ...
    /*
    Now we encode it with CKKSEncoder. The floating-point coefficients of `input'   scale:编码精度
    ...
    In CKKS the message is stored modulo coeff_modulus (in BFV it is stored modulo	在CKKS中，message存储为模coeff_modulo(在BFV中，消息存储为模plain_modulo)，因此缩放后的消息不能太接近coeff_modulo。
