...
static string loadCipherFromFile(shared_ptr<SEALContext> context, const string &filePath)
{
	ifstream file(filePath, ios_base::binary);
	string str = "";
	if (file.is_open())
...
std::string CryptoContext::loadCiphertext(const std::string &ciphertextPath) {
	return loadCipherFromFile(m_context, ciphertextPath);
}
