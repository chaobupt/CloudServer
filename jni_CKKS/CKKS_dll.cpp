...
(JNIEnv *env, jclass jclass, jlong contextHandle) {
...
(JNIEnv *env, jclass jclass, jlong contextHandle, jdoubleArray input, jstring ciphertextPath) {
...
(JNIEnv *env, jclass jclass, jlong contextHandle, jstring input) {
...
(JNIEnv *env, jclass jclass, jlong contextHandle, jstring publicKeyPath, jstring secretKeyPath) {
...
extern "C" JNIEXPORT jstring JNICALL Java_info_androidhive_webmobilegroupchat_JniUtils_nativeLoadCiphertext
(JNIEnv *env, jclass jclass, jlong contextHandle, jstring ciphertextPath) {
	const char *ciphertext = env->GetStringUTFChars(ciphertextPath, nullptr);
	CryptoContext *context = reinterpret_cast<CryptoContext*>(contextHandle);
	jstring ciphertextString = env->NewStringUTF(context->loadCiphertext(ciphertext).c_str());
	env->ReleaseStringUTFChars(ciphertextPath, ciphertext);
	return ciphertextString;
}
